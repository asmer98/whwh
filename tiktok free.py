...
an2 = datetime.datetime(2025, 2, 12, 12, 00, 00)#تحكم بالوقت
if an > an2 or an == an2:
        exit("انتهت مدة التفعيل المجاني للاشتراك راسل المطور احمد خان AHMED_KHANA")
else:
        pass
import os    
...
def Gmail(email):
    ...
    if "password"in response:
        ...
        ge+=1
        sys.stdout.write(f'''\r{F}True {HH}: {M}{ge} ~ {Z}False {HH}: {M}{bt} ~ {X}Not {HH}: {M}{be} {G}'''),sys.stdout.flush()
    else:
